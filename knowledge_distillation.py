...
def train_epoch_distill(teacher_model, student_model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem,
    ...
    if opts.multi_teacher and opts.distill_distribution:
        ...
        if opts.adaptive_prob and epoch >=opts.start_adaptive_epoch and opts.change:
            if opts.random_adaptive_prob!=0: # randomly choose the equal prob mode OR the adaptive mode
                ...
                class_num = np.random.choice([20, 50, 100], size=1, p=opts.teacher_prob)
                load_path = 'teacher/{}{}/{}{}-uniform-epoch-99.pt'.format(opts.problem, class_num.item(),opts.problem, class_num.item())
                #load_path = opts.load_path_multi[class_type.item()]
                print('  [*] Loading data from {}, prob: {}'.format(load_path, opts.teacher_prob))
        else: # directly use the equal prob mode
            #class_type = np.random.choice(['uniform','cluster','mixed'],1)
            class_type=np.array("uniform")
            load_path = 'teacher/{}{}/{}{}-uniform-epoch-99.pt'.format(opts.problem, 20,opts.problem, 20)
            print('  [*] Loading data from {}'.format(load_path))
    ...
    if opts.multi_test:
        avg_reward_uniform_20 = validate(student_model, val_dataset[0], opts, print_log='uniform_20')
        avg_reward_uniform_50 = validate(student_model, val_dataset[1], opts, print_log='uniform_50')
        avg_reward_uniform_100 = validate(student_model, val_dataset[2], opts, print_log='uniform_100')
        if not opts.no_tensorboard:
            tb_logger.log_value('val_avg_reward_uniform', avg_reward_uniform_20, step)
            tb_logger.log_value('val_avg_reward_cluster', avg_reward_uniform_50, step)
            tb_logger.log_value('val_avg_reward_mixed', avg_reward_uniform_100, step)
    else:
    ...
    if opts.adaptive_prob:
        return [avg_reward_uniform_20 ,avg_reward_uniform_50,avg_reward_uniform_100]
...
def train_batch_distill(
    ...
    if opts.multi_teacher:
        ...
        if opts.meaningful_KLD:
            ...
            if opts.KDloss=="PK_Temperature":
                eps=1e-6
                T=opts.distill_temperature
                print(teacher_logp.shape)
                teacher_logp, student_logp = (teacher_logp.exp() + 0.00001).log(), (student_logp.exp() + 0.00001).log()
                feat_s_norm=torch.sqrt(torch.sum(student_logp ** 2, dim=1, keepdim=True))
                student_logp = student_logp / (feat_s_norm + eps)
                teacher_logp_norm = torch.sqrt(torch.sum(teacher_logp ** 2, dim=1, keepdim=True))
                teacher_logp = teacher_logp / (teacher_logp_norm + eps)
                feat_s_cos_sim = torch.matmul(student_logp, student_logp.transpose(1, 2))
                feat_t_cos_sim = torch.matmul(teacher_logp, teacher_logp.transpose(1, 2))
                ...
                #feat_s_cond_prob = F.log_softmax(feat_s_cond_prob/T,dim=1)
                #feat_t_cond_prob = F.log_softmax(feat_t_cond_prob/T,dim=1)
                #soft_loss = torch.mean(feat_t_cond_prob * torch.log((feat_t_cond_prob + eps) / (feat_s_cond_prob + eps)))
                soft_loss = F.kl_div(
                F.log_softmax(feat_s_cond_prob / T, dim=1),
                F.log_softmax(feat_t_cond_prob / T, dim=1),
                reduction='sum',
                log_target=True
            ) * (T * T) / student_logp.numel()
