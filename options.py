...
def get_options(args=None):
    ...
    parser.add_argument("--KL_adapative",action='store_true',default=False,help="select adapative stratege")
    parser.add_argument("--change",action='store_true',default=False,help="teacher model size")
    parser.add_argument("--KDloss",type=str,help="distillation use loss")
    ...
    parser.add_argument('--start_adaptive_epoch',type=int, default=1000)
    ...
    parser.add_argument('--student_n_encode_layers', type=int, default=3,
    ...
    parser.add_argument('--distill_temperature', type=int, default=250)
    ...
    if opts.multi_test:
        if opts.problem == 'tsp':
            opts.val_dataset_uniform_20 = 'data/tsp/tsp_uniform20_1000_seed1234.pkl'
            opts.val_dataset_uniform_50 = 'data/tsp/tsp_uniform50_1000_seed1234.pkl'
            opts.val_dataset_uniform_100 = 'data/tsp/tsp_uniform100_1000_seed1234.pkl'
    ...
    if opts.distillation:
        if opts.distill_distribution:
            if not opts.load_path_uniform:
                opts.load_path_uniform = 'teacher/{}{}/{}{}-uniform-epoch-99.pt'.format(opts.problem, 100, opts.problem, 100)
                print("Using given Uniform teacher")
            if not opts.load_path_cluster:
                opts.load_path_cluster = 'teacher/{}{}/{}{}-cluster-epoch-99.pt'.format(opts.problem, 100, opts.problem, 100)
                print("Using given Cluster teacher")
            if not opts.load_path_mixed:
                opts.load_path_mixed = 'teacher/{}{}/{}{}-mixed-epoch-99.pt'.format(opts.problem, 100, opts.problem, 100)
