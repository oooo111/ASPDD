...
from knowledge_distillation import train_epoch_distill, validate, get_inner_model
...
def run(opts):
    ...
    if opts.multi_test:
        val_dataset = [problem.make_dataset(
            size=20, num_samples=opts.val_size, filename=opts.val_dataset_uniform_20,
            distribution='uniform', n_cluster=opts.n_cluster, n_cluster_mix=opts.n_cluster_mix),
            problem.make_dataset(
                size=50, num_samples=opts.val_size, filename=opts.val_dataset_uniform_50,
                distribution='uniform',n_cluster=opts.n_cluster, n_cluster_mix=opts.n_cluster_mix),
            problem.make_dataset(
                size=100, num_samples=opts.val_size, filename=opts.val_dataset_uniform_100,
                distribution='uniform', n_cluster=opts.n_cluster, n_cluster_mix=opts.n_cluster_mix)]
    else:
